...
import { CheckCircle, Download, Ticket, ArrowLeft, CalendarPlus, Sparkles } from 'lucide-react'
        ...
        <Card variant="elevated" className="max-w-2xl mx-auto text-center animate-scale-in-bounce premium-border shine">
          <div className="mb-8 animate-fade-in-up">
            <div className="relative inline-block mb-6">
              <CheckCircle className="w-28 h-28 text-gold mx-auto animate-scale-in-bounce" />
            ...
            <h1 className="font-display text-4xl md:text-6xl font-bold text-ivory mb-4 text-shadow-lg">
              Payment <span className="gradient-text italic">Successful!</span>
            </h1>
          ...
          {tickets.length > 0 && (
            <div className="space-y-4 mb-6">
              {tickets.map((ticket) => (
                <div
                  key={ticket.id}
                  className="p-6 bg-gold/5 border border-gold/20 rounded-xl hover:shadow-glow transition-all duration-300"
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-left">
                      <h3 className="font-display font-semibold text-ivory">{ticket.event.title}</h3>
                      <p className="text-sm text-taupe">
                        {new Date(ticket.event.date).toLocaleDateString()}
                      </p>
                    </div>
                    {ticket.qrCodeImage && (
                      <div className="p-4 bg-white rounded-lg border-2 border-gold shadow-2xl">
                        <img
